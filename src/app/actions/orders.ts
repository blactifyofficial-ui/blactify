...
export async function generateAdminOrderLabel(id: string, token?: string) {
    try {
        const { verifyActionAdminAuth } = await import("@/lib/auth-server");
        await verifyActionAdminAuth(token);
        ...
        if (error || !order) throw new Error("Order not found.");
        if (!order.tracking_id) throw new Error("A tracking sequence (AWB) has not yet been assigned to this order.");

        const { generateLabel } = await import("@/actions/delhivery");
        const labelResult = await generateLabel(order.tracking_id);

        if (labelResult.success && labelResult.packages && labelResult.packages.length > 0) {
            const pdfUrl = String(labelResult.packages[0].pdf_url);
            ...
            await supabaseAdmin
                ...
                .update({
                    payment_details: {
                        ...((order.payment_details as Record<string, unknown>) || {}),
                        shipping: {
                            ...(((order.payment_details as Record<string, Record<string, unknown>>)?.shipping) || {}),
                            label_url: pdfUrl,
            ...
            return { success: true, label_url: pdfUrl };
        } else {
            throw new Error(labelResult.message || "Failed to generate label from Delhivery.");
        }
    } catch (err: unknown) {
