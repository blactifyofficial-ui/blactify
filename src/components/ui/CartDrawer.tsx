...
export function CartDrawer({ isOpen, onClose, onAuthRequired }: {
    ...
    return (
        <>
            ...
            <div
                className={cn(
                    "fixed inset-y-0 right-0 z-[80] w-full max-w-md bg-black/80 backdrop-blur-xl border-l border-white/10 shadow-3xl transition-transform duration-300 ease-in-out",
            ...
            >
                <div className="flex h-full flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-6">
                        {items.length === 0 ? (
                            <div className="flex h-full flex-col items-center justify-center text-center">
                                ...
                                <p className="text-zinc-500 font-medium uppercase text-[10px] tracking-widest">Your bag is empty</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="flex flex-1 flex-col justify-between">
                                            <div>
                                                <div className="flex justify-between">
                                                    ...
                                                    <button onClick={() => removeItem(item.cartId || item.id)} className="text-xs font-bold text-zinc-500 uppercase tracking-widest hover:text-white">Remove</button>
                                                </div>
                                                <p className="text-xs text-zinc-500 uppercase tracking-widest">{item.categories?.name || item.category}</p>
                                                {item.size && (
                                                    <p className="text-xs text-zinc-500 uppercase tracking-widest">Size: {item.size}</p>
                                                )}
                                            </div>
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-[13px] font-medium text-white">₹{((item.price_offer || item.price_base) * item.quantity).toLocaleString()}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    {items.length > 0 && (
                        <div className="border-t border-white/10 px-6 py-6 bg-black/60 backdrop-blur-lg">
                            <div className="mb-6 flex items-center justify-between">
                                <span className="text-sm font-bold uppercase tracking-widest text-zinc-500">Total</span>
                                <span className="text-xl font-medium text-white">₹{Math.round(getTotalPrice()).toLocaleString()}</span>
                            </div>
                                <button
                                    onClick={handleCheckout}
                                    disabled={items.length === 0 || isCheckingOut}
                                    className="w-full rounded-md bg-white py-4 text-xs font-bold uppercase tracking-widest text-black active:scale-[0.98] transition-all disabled:opacity-50"
                                >
